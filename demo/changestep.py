...
def get_sleep_time():
    ...
    # 第二天12点时间戳
    tomorrow_run_time = int(time.mktime(time.strptime(str(tomorrow), '%Y-%m-%d'))) + 43200
...
def do_it(steps, username, password):
    try:
        # 修改步数结果
        result = LexinSport(username, password, int(steps)).change_step()
        return result
