...
def get_total_count():
    ...
    :return:
    """
    sql = "select count(*) from USA_EN_INFO_1116"
    cursor.execute(sql)
    rst = cursor.fetchone()
    print(rst[0])
    if rst is None:
...
def get_tweet_by_page(start, pagesize):
    ...
    :return:
    """
    sql = "select * from USA_EN_INFO_1116 limit %s offset %s "
...
def limit_offset_query():
    ...
    for i in range(int(total_page) + 1):# 遍历每一页
        ...
        if (len(result_list) > 0):
            ...
            entity_file = open('./entity_full_test.json', 'a+', encoding='utf-8')
            ...
            for row in result_list:
                ...
                # twitter_username 36
                dic['fb账号ID'] = '' if ( row[37] == None ) else row[37]
                # linedin_in 38
